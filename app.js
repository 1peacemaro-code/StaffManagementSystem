...
function formatCardTitle(dateValue){
  const d=new Date(dateValue+"T12:00:00");
  const w=["日","月","火","水","木","金","土"][d.getDay()];
  return `${d.getMonth()+1}/${d.getDate()}（${w}）の勤務`;
}
...
function renderCalendar(){
  ...
  const first=new Date(parsed.year,parsed.month-1,1,12,0,0);
  const last=new Date(parsed.year,parsed.month,0,12,0,0);
...
async function loadMasters(){
  try{
    ...
    if(data.teachers&&data.teachers.length){
      ...
      teacherSelect.innerHTML='<option value="">先生が登録されていません</option>';
    }
...
function updateCounter(){
  ...
  dayCounter.textContent=`${count} / ${MAX_DAYS}日 選択中`;
}
...
function sortDayCards(){
  ...
  cards.sort((a,b)=>a.querySelector(".work-date").value.localeCompare(b.querySelector(".work-date").value));
  cards.forEach(card=>dayCards.appendChild(card));
}

function addDayCard(dateValue){
  if([...dayCards.querySelectorAll(".work-date")].some(i=>i.value===dateValue)){
    updateCalendarSelected();
    return;
  }

  if(dayCards.querySelectorAll(".day-card").length>=MAX_DAYS){
    alert("追加できる日付は最大27日です。");
    return;
  }
  ...
  card.querySelector(".card-title").textContent=formatCardTitle(dateValue);
  ...
  // ここが重要：
  // 以前はここで自動スクロールしていましたが、Ver.6.2ではしません。
  // 先に日付だけ複数選び、あとから下で1つずつ入力できます。
}
...
function getCardData(card){
  ...
  return{
    date:card.querySelector(".work-date").value,
    workStart:card.querySelector(".work-start").value,
    workEnd:card.querySelector(".work-end").value,
    normalLessons,
    standard
  };
}
...
function collectData(){
  return{
    ...
    days:[...dayCards.querySelectorAll(".day-card")].map(getCardData)
  };
}
...
function validateData(data){
  ...
  if(data.days.length===0)errors.push("日付を1つ以上選択してください。");
...
function buildPreview(){
  previewOutput.textContent=JSON.stringify(collectData(),null,2);
}
...
async function checkSubmitStatus(){
  ...
  if(!teacher||!month){
    submitStatus.className="status neutral";
    submitStatus.textContent="先生名と対象月を選択すると提出状況が表示されます。";
    return;
  }
  ...
  try{
    ...
    submitStatus.textContent="提出状況の取得に失敗しました。";
  }
}
...
async function submitToGas(){
  ...
  try{
    ...
    setStatus("success","送信しました。最新提出として保存されます。");
    alert("送信しました。");
    setTimeout(checkSubmitStatus,1200);
  }catch(e){
